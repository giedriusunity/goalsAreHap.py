...
class ZeroCases:
    def __init__(self, filterName, filter, beyond):  #this is probably not the way assarys in python classes should be but hey, it works
        self.filterName = []
        self.filter = []
        self.beyond = beyond
...
def CheckGoals(data):
    ...
    searchOldestUpdate = urllib.request.quote(assignedFilter)  #Quotes, brackets and other signs need to be formatted for web queries otherwise they fail
    ...
    tree = ET.ElementTree(ET.fromstring(content))  #There might be better plugins for decoding XMLs but this is inbuilt and works well enoguh
    ...
    offenderCases = [Cases(None, None, None, None, None, None)] * len(oldestUpdateLastEdited)  #Array for cases that are bad
    ...
    for i in range(0, len(oldestUpdateLastEdited)):
        ...
        dateFormatted = datetime.strptime(dateStripped, "%Y-%m-%d")  #Date from fogbugz comes in a ISO 8601 Notation, needs to be fixed up for comparison
    ...
    offenderCount = 0
    activeCases = 0
    offenderCases = [Cases(None, None, None, None, None, None)] * len(oldestUpdateLastEdited)  #Resetting bad case array
    ...
    for i in range(0, len(oldestUpdateLastEdited)):
        ...
        if i == 0 or goalCount - (dateFormatted - goal).days > goalValues.g3: #This is for all cases that are old and not resovled
        ...
        if ((dateFormatted - goal).days < 0 \
                ...
                and "105" in oldestUpdatedMilestone[i].text):  #This is for cases that are returned as converted but triage back from devs. milestone = 105 is Triage
            goalIsMet = False
...
#                  caseValues[i].lastEdited)
    ...
    assignedTo =\
        urllib.request.quote('assignedTo:"QA Incoming" AND category:"qa incoming incident" and status:active AND(')  #First we need to search all cases through all filters in the team
    ...
    caseValues = [0] * (len(oldestAllFiltersBugId) + 999)  #Well this contradicts the comments above but honestly with how retarded fogbugz searches are this is better

    totalCases = 0

    zeroTracker = ZeroCases([], [], 0)

    for filterIndex in range(0, len(newFilters)):  #Again, there probably is a better way to make arrays inside custom classes
    ...
    for filterIndex in range(0, len(newFilters)):
        ...
        for i in range(0, len(oldestFiltersBugId)):
            ...
            if (dateFormatted - zeroGoal).days < -1:
                ...
                zeroTracker.filter[filterIndex] = zeroTracker.filter[filterIndex] + 1  #How many cases each filter older than 1 day has
    ...
    potentialOffenderCases = [Cases("", "", "", "", "", "")] * casesBeyondBottleneck  #If there are less that 10 cases as offenders, also populate sheet with oldest cases for Goal4
    ...
    if caseBottleneckAmount < 10 and totalCases != 0:
        ...
        for index in range(0, loopLen):
            ...
            if index == 0:
                potentialOffenderCases[index].assignedTo = FindTurn(potentialOffenderCases[index],  #Find whose turn is it to take the next case bad for Goal 4
                                                                    data,)


    goalValues.offender4 = offenderCount
    if goalValues.offender3 == 0 and goalValues.offender2 == 0 and goalValues.offender1 == 0:
        filteredPotentialOffendrers = RecursiveRemoveDupes(offenderCases, potentialOffenderCases)  #If a cases appears cross filter, e.g "XR" for iOS and VR
...
def FindTurn(original, data):
    ...
    for index in range(0, len(data["turnOrder"])):
        ...
        if original.assignedTo == data["turnOrder"][index]["filterName"]:
            ...
            formatted = original.assignedTo + "(" + data["turnOrder"][index]['owners'][int(turnOrder)] + ")"  #E.g iOS to iOS(John Johnson)

            if original.bugId != turn["lastCase"]:  #Last one that was oldest is stored in a json and is used for comparison
                ...
                if data["turnOrder"][index]["owners"][int(turnData["turn"][index])] == personTurn:  #Is the person the last oldest case is assigned to the person whose turn it was. Check keyValue for structure
...
def ReportGoals(goalValues, regularDay, downIndex, data):
    ...
    newSTR = str(values[len(values) - 2])  # In the google sheet, current will be 1 less than row length, but highest number of "week" will be 2 less
    ...
    for i in range(0, len(zeroTracker.filter)):
        if zeroTracker.filter[i] == 0:  #Marks that that team has reached 0 cases older than a day
            if zeroCaseData["datesReached"][i] == "0":
                ...
                dateStripped = datetime.strptime(dateStripped, '%Y-%m-%d').strftime('%m/%d/%Y')  #Fogbugz takes date in this format.
                zeroCaseData["datesReached"][i] = dateStripped

                timeFormat = "" + str(datetime.now().hour-1) + ":" + str(datetime.now().minute)  #its queries also run on UK time
            ...
            for y in range(0, len(zeroCaseData["filters"][i]["owners"])):
                if y < len(zeroCaseData["filters"][i]["owners"])-1:
                    teamEdited = teamEdited + "editedBy:'" + zeroCaseData["filters"][i]["owners"][y] + "' OR "  #Combining names of people into one query
                else:
...
def CheckForAlreadyEdited(index, bugIDs, cases):
    ...
    for y in range(0, len(bugIDs)):
        notDupe = True
        for i in range(0, len(cases["casesChecked"])):  #Checks through all cases done up to this point. Now this can be a bit wasteful as it checks each case against 25k+ cases so the array might need to be reset after some time
...
def main():
    ...
    while True:
        ...
        if 7 <= curDate.hour < 23:  #Runs only during work hours to not put pressure on fogbugz when not necessary
            ...
            if date.today().weekday() == int(data['updateTime'][0]) and hasNoBeenLockedThisWeek:  #This activates when a week has passed and needs to move to another line
